...
func TestStartCompletionAppliesCurrentInputFileGlobally(t *testing.T) {
	...
	if got := ds.uploads[0].Filename; got != "HRIDS_HISTORY.txt" {
		t.Fatalf("upload filename=%q want HRIDS_HISTORY.txt", got)
	}
	...
	if !start.Request.CurrentInputFileApplied || !strings.Contains(start.Request.PromptTokenText, "# HRIDS_HISTORY.txt") {
